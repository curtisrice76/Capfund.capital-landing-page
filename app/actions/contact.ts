...
export async function submitContactForm(formData: FormData) {
  try {
    ...
    const transporter = nodemailer.createTransporter({
      service: "gmail",
      auth: {
        user: process.env.GMAIL_USER, // info@capfund.capital
    ...
    await transporter.sendMail({
      from: `"CAP Website" <${process.env.GMAIL_USER}>`,
    ...
    return {
      success: false,
      error:
        "There was an error sending your message. Please try again or contact us directly at info@capfund.capital.",
    }
  }
}
